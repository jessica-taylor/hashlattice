...
function Run(name) {
  ...
  this.stopTime = this.startTime + 10000;
}
...
Profiler.prototype.makeGanttChart = function(div) {
  ...
  var minTime = U.min(U.pluck(self.runs, 'startTime'));
  var data = U.map(self.runs, function(run, i) {
  ...
  var options = {
    xaxis: {
      ticks: U.map(self.runs, function(run, i) { return [i, run.name]; })
    }
  };
