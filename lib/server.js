...
Server.prototype._rawEvalComputation = function(hash, comp, callback) {
  ...
  var api = {
    cl: CompLib,
    getHash: new Code.AsyncFunction(_.bind(self.getHash, self)),
    getHashData: new Code.AsyncFunction(_.bind(self.getHashData, self)),
    evalComputation: new Code.AsyncFunction(_.bind(self.evalComputation, self))
  };
...
Server.prototype.getVarEvaluator = function() {
  ...
  return {
    ...
    merge: function(varSpec, value1, value2, callback) {
      ...
      getVarObj(varSpec, function(err, varObj) {
        console.log('varObj', varObj.merge.async);
        if (err) {
          callback(err);
        } else {
