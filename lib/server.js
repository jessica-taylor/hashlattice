...
function Server(kwargs) {
  ...
  this.hashDataStore = kwargs.hashDataStore || new Store.MemoryStore();
...
Server.prototype.getHashData = function(hash, callback) {
  ...
  this.hashDataStore.get(hash, callback);
};
...
Server.prototype.putHash = function(value, callback) {
  ...
  if (Value.isData(value)) {
    self.hashDataStore.put(Value.hashData(value), value, callback);
  } else {
