...
function Server(kwargs) {
  ...
  if (kwargs.varEvaluatorToVarStore) {
    this.varStore = kwargs.varEvaluatorToVarStore(this.getVarEvaluator());
  } else {
    this.varStore = new Store.MergingVarStore(this.getVarEvaluator(), new Store.MemoryStore());
  }
}
