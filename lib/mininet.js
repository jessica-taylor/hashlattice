...
function Mininet(topology) {
  ...
  self.eventuallyExecute('py sys.stderr.write("HOSTS " + __import__("json").dumps([{"name": h.name, "ip": h.IP(), "port": h.ports.values()} for h in net.hosts]) + "\\n")');
  ...
  Carrier.carry(self.mn.stderr).on('line', function(line) {
    if (line.substring(0, 5) == 'HOSTS') {
      ...
      if (line.indexOf('Traceback') > -1 || line.indexOf('Unknown') > -1) {
...
Mininet.prototype.getIP = function(virtualHost, callback) {
  ...
  if (self.hostToIP === null) {
    // wait for self.hostToIP to be initialized
    setTimeout(function() { self.getIP(virtualHost, callback); }, 30);
  } else if (host in self.hostToIP) {
    callback(null, self.hostToIP[host]);
  } else {
    callback('Error: host ' + host + ' does not exist in this topology');
  }
};

Mininet.prototype.getPort = function(virtualHost, callback) {
  var self = this;
  var host = 'h' + virtualHost;
  if (self.hostToPorts === null) {
    // wait for self.hostToPorts to be initialized
    setTimeout(function() { self.getPort(virtualHost, callback); }, 30);
  } else if (host in self.hostToPorts) {
    callback(null, self.hostToPorts[host][0]);
  } else {
    callback('Error: host ' + host + ' does not exist in this topology.');
  }
}

Mininet.prototype.eventuallyExecute = function(cmd) {
  var self = this;
  if (self.canExecute) {
    self.mn.stdin.write(cmd + '\n');
    self.canExecute = false;
    setTimeout(function() { self.canExecute = true; }, 100);
  } else {
    setTimeout(function() { self.eventuallyExecute(cmd); }, 50);
  }
};
...
Mininet.prototype.runCommand = function(virtualHost, cmd) {
  ...
  ChildProcess.spawn('mkfifo', [stdoutPipeName, stderrPipeName], {
    ...
    assert.equal(0, code);
    self.eventuallyExecute('h' + virtualHost + ' ' + cmd + ' < /dev/null > ' + stdoutPipeName + ' 2> ' + stderrPipeName + ' &');
  });
