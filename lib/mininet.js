...
Mininet.prototype.runCommand = function(virtualHost, cmd) {
  var self = this;
  var pipeName = '/tmp/hashlattice_mininet_fifo_' + new Date().getTime();
  ChildProcess.spawn('mkfifo', [pipeName], {
    stdio: ['ignore', 'ignore', process.stderr]
  }).on('exit', function(code) {
    assert.equal(0, code);
    self.mn.stdin.write('h' + virtualHost + ' ' + cmd + ' > ' + pipeName + ' &\n');
  });
  var readProcess = ChildProcess.spawn('cat', [pipeName], {
