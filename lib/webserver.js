...
WebServer.prototype.handleAPI = function(hash, funcName, args, req, res) {
    ...
    switch(funcName) {
        ...
        case 'apiObject':
            ...
            // Check that the hash is actually in the cache, encode the API
            // object, and send it to the client.
            if (self.apiCache[hash]) {
              if(!self.functionserver[hash]) {
                self.functionserver[hash] = new FunctionServer();
              }
                var binApiObject =
                    self.functionserver[hash].encodeValue(self.apiCache[hash]);
                res.writeHead(200, {'Content-Type':
                    'application/octet-stream'});
    ...
    this.httpserver = http.createServer(function(req, res) {
        ...
        if (splitUrl.length < 2 || splitUrl[1] === '') {
            ...
            self.server.getHash(hashBuffer, function(err, getData) {
                if (!err) {
                    var data = getData(hl);
                    // Cache the API for this HashLattice site.. 
                    self.apiCache[hash] = data.api;
