  ...
  this.httpserver = Http.createServer(function(req, res) {
    ...
    if (splitUrl.length < 2 || splitUrl[1] === '') {
      ...
      var hl = {
        ...
        putHashData: new Code.AsyncFunction(_.bind(self.server.putHashData, self.server)),
        putHashDataSplit: new Code.AsyncFunction(_.bind(self.server.putHashDataSplit, self.server)),
        getVar: new Code.AsyncFunction(_.bind(self.server.getVar, self.server)),
        putVar: new Code.AsyncFunction(_.bind(self.server.putVar, self.server)),
        getLocal: new Code.AsyncFunction(function(key, cb) {
          var newKey = Value.hashData({ hash: hashBuffer, key: key });
          self.localStore.get(newKey, cb);
        }),
        putLocal: new Code.AsyncFunction(function(key, value, cb) {
          var newKey = Value.hashData({ hash: hashBuffer, key: key });
