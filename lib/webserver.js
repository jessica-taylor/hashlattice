      ...
      Utilities.readAllFromStream(req, function(err, fnArgsBinary) {
        if (err) {
          ...
          wait.launchFiber(function() {
            var result = fn.async.apply(null, fnArgs.concat([function(err, data) {
              if (!err) {
  ...
  this.httpserver = Http.createServer(function(req, res) {
    ...
    if (splitUrl.length < 2 || splitUrl[1] === '') {
      ...
      var hl = {
        ...
        putHashData: new Code.AsyncFunction('putHashData', _.bind(self.server.putHashData, self.server)),
        putHashDataSplit: new Code.AsyncFunction('putHashDataSplit', _.bind(self.server.putHashDataSplit, self.server)),
        getVar: new Code.AsyncFunction('getVar', _.bind(self.server.getVar, self.server)),
        putVar: new Code.AsyncFunction('putVar', _.bind(self.server.putVar, self.server)),
        getLocal: new Code.AsyncFunction('getLocal', function(key, cb) {
          var newKey = Value.hashData({ hash: hashBuffer, key: key });
          self.localStore.get(newKey, cb);
        }),
        putLocal: new Code.AsyncFunction('putLocal', function(key, value, cb) {
          var newKey = Value.hashData({ hash: hashBuffer, key: key });
