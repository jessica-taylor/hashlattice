...
function logDistance(hash1, hash2) {
  ...
  var bitsInCommon = 0;
  for (var i = KEY_BITS/8 - 4; i >= 0; i -= 4) {
    var xor = hash1.readUInt32LE(i) ^ hash2.readUInt32LE(i);
    if (xor == 0) {
      bitsInCommon += 32;
    } else {
      for (var j = 31; j >= 0; j--) {
...
function sortPeersByDistance(origin, peers) {
  ...
  peers.sort(function(p1, p2) {
    ...
    for (var i = hash.length - 4; i >= 0; i -= 4) {
      var hashword = hash.readUInt32LE(i);
      var p1word = p1hash.readUInt32LE(i);
      var p2word = p2hash.readUInt32LE(i);
