...
function logDistance(hash1, hash2) {
  ...
  for (var i = 0; i < KEY_BITS/8; i += 4) {
    var xor = (hash1.readUInt32BE(i) ^ hash2.readUInt32BE(i)) >>> 0;
    if (xor == 0) {
      bitsInCommon += 32;
    } else {
      for (var j = 31; j >= 0; j--) {
...
function uniquePeers(peers) {
  return U.uniq(peers, false, function(p) { return Value.encodeValue(p.getSpec()).toString('hex'); });
}
...
function sortPeersByDistance(origin, peers) {
  ...
  peers.sort(function(p1, p2) {
    ...
    for (var i = 0; i < origin.length; i += 4) {
      ...
      var p1xor = (hashword ^ p1word) >>> 0;
      var p2xor = (hashword ^ p2word) >>> 0;
      if (p1xor < p2xor) {
        return -1;
      }
      if (p2xor < p1xor) {
