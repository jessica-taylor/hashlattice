...
function Node(kwargs) {
  ...
  for (var i = 0; i < self.bootstraps.length; i++) {
    self.peerSet.insert(new Peer(self, self.bootstraps[i]));
  }
}
...
Node.prototype.getSelfPeer = function(callback) {
  ...
  if (self.selfPeer) {
    ...
    function pingPeers() {
      if (i >= peers.length) {
        callback(new Error('no one knows who I am :('));
      } else {
...
Node.prototype.storeIterative = function(query, value, callback) {
  ...
  self.findClosestNodesIterative(queryHash, function(err, closestPeers) {
    if (err) {
      ...
      console.log('storing in', closestPeers);
...
Node.prototype.handle_find = function(reqObj, callback) {
  ...
  self.queryStore.get(queryHash, function(err, data) {
    console.log('the queryStore is', self.queryStore);
    console.log('get', queryHash.toString('hex'), err, data);
    var result = {};
    if (!err) {
      result.value = data;
    }
  ...
  self.queryStore.get(queryHash, function(err, data) {
    ...
    if (!Value.valuesEqual(oldValue, newValue)) {
      self.queryStore.put(queryHash, newValue, function(err) {
        console.log('stored in queryStore', self.queryStore);
...
Node.prototype.putHashData = function(value, callback) {
  ...
  Async.parallel([
    function(cb) { 
      self.hashDataStore.putHashData(value, cb);
    },
    function(cb) {
