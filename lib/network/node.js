  ...
  self.findIterative(query, function(err, varpeers) {
    ...
    if (err) {
      ...
      Async.map(varpeers, function(p, cb) {
        p.getvar(variable, function(err, value) {
          if (value.value) {
...
Node.prototype.putVar = function(variable, value, callback) {
  ...
  Async.parallel([
    function(cb) {
      ...
      self.varStore.putVar(variable, value, function(err, newValue) {
        if (err) {
          cb(err);
        } else {
          self.findIterative(query, function(err, varpeers) {
            ...
            if (err) {
              ...
              Async.map(varpeers, function(p, cb) {
                p.putvar(variable, newValue, function() { cb(); });
              }, function(err, results) {
                assert(!err, err);
                cb(null, newValue);
              });
