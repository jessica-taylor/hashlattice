...
function Node(kwargs) {
  ...
  self.hashDataStore = kwargs.hashDataStore || new Store.MemoryStore();
  self.varStore = kwargs.varStore || new Store.MemoryStore();
...
Node.prototype.handle_gethashdata = function(reqObj, callback) {
  ...
  self.hashDataStore.get(reqObj.hash, function(err, value) {
    var result;
    if (err) {
      if (err == 'not found') {
        result = {found: false};
      ...
      function askSeeders() {
        if (i >= seeders.length) {
          ...
          new Peer(self, seeders[i]).gethashdata(function(err, data) {
            if ('value' in data && Value.hashData(data.value).toString('hex') ==key.toString('hex')) {
              callback(null, data);
            } else {
              i++;
              askSeeders();
...
Node.prototype.put = function(key, value, callback) {
  ...
  Async.parallel([
    function(cb) { 
      self.hashDataStore.put(queryHash, value, cb);
    },
    function(cb) {
