...
KBucket.prototype.insert = function(peer) {
  if (!U.any(this.peers, function(p) {
        return Value.valuesEqual(p.getSpec(), peer.getSpec());
      })) {
...
function PeerSet(kwargs) {
  ...
  if (!this.origin) {
    ...
    this.kbuckets = U.times(NetUtil.KEY_BITS + 1, function() { return new KBucket(); });
  }
}

PeerSet.prototype.insert = function(peer) {
  if (this.kbuckets.length < NetUtil.KEY_BITS) {
...
PeerSet.prototype.allPeers = function() {
  return U.flatten(U.map(this.kbuckets, function(kb) { return kb.peers; }), true);
};
