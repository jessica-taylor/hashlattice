...
try {
  ...
  try {
    // this works outside the browser if wrtc is installed

    // use require2 to avoid confusing browserify
    var require2 = require;

    // from https://github.com/peers/peerjs/issues/103
    window = {};
    window.BlobBuilder = require2("BlobBuilder");
    location = {};
    location.protocol="http";

    BinaryPack = require2("binary-pack");
    XMLHttpRequest = require2("xmlhttprequest").XMLHttpRequest;

    var wrtc = require2('wrtc');
    RTCPeerConnection = wrtc.RTCPeerConnection;
    RTCSessionDescription = wrtc.RTCSessionDescription;
    RTCIceCandidate = wrtc.RTCIceCandidate;

    WebSocket = require2('ws');
    Peer = require2("./peerjs").Peer;
  } catch (err) {
    console.warn('warning: no webrtc found', err);
  }
}
...
function WebRTCTransport() {
  var self = this;
  self.peer = new Peer({key: 'j06d4hq2wat9be29'});
