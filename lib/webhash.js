(function() {
  ...
  function getResponseByPath(pathname, callback) {
    ...
    if (!match) {
      ...
      var hl = {
        path: restPath,
        putHashData: _.bind(window.server.putHashData, window.server),
        putHashDataSplit: _.bind(window.server.putHashDataSplit, window.server),
        getVar: _.bind(window.server.getVar, window.server),
        putVar: _.bind(window.server.putVar, window.server),
        getLocal: _.bind(localStore.get, localStore),
        putLocal: _.bind(localStore.put, localStore),
