...
function FileStore(directory) {
  ...
  this.made = false;
  console.log('mkdirping');
  mkdirp(this.directory, function(err) {
    console.log('mkdirped');
    assert(!err, err);
    this.made = true;
  });
}
...
FileStore.prototype.getFile = function(key, callback) {
  var self = this;
  if (self.made) {
    callback(null, Path.join(self.directory, key.toString('hex')));
  } else {
          ...
          function storeUsingOldValue(oldValue) {
            self.varEvaluator.merge(varSpec, oldValue, value, function(err, merged) {
              if (err) {
                ...
                self.valueStore.put(varSpecBinary, merged, function(err) {
                  if (err) {
                    release();
                    callback(err);
                  } else {
...
function LayeredVarStore(varEvaluator, store1, store2) {
  this.varEvaluator = varEvaluator;
  this.store1 = store1;
  this.store2 = store2;
}
...
LayeredVarStore.prototype.getVar = function(varSpec, callback) {
  var self = this;
  self.store1.getVar(varSpec, function(err1, v1) {
    self.store2.getVar(varSpec, function(err2, v2) {
      if (err1) {
        if (err2) {
          self.varEvaluator.defaultValue(varSpec, callback);
        } else {
          callback(null, v2);
        }
      } else {
        if (err2) {
          ...
          self.varEvaluator.merge(varSpec, v1, v2, callback);
        }
      }
    });
  });
...
function layerVarStores() {
  var evaluator = arguments[0];
  var store = arguments[1];
  for (var i = 2; i < arguments.length; i++) {
    store = new LayeredVarStore(evaluator, store, arguments[i]);
  }
  return store;
}
