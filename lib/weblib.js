
(function() {
  var Value = require('./value');
  var $ = require('jquery');

  // This doesn't actually work yet.  It's mostly for reference for the web server.

  function getHashURL() {
    var hash = window.location.pathname;
    hash = hash.match(/\/(\w*)\//);
    return hash[1];
  }

  function decodeData(binary) {
    return Value.decodeValue(binary, function(id) {
      var url = '/' + getHashURL() + '/_api/callFunction/' + id + '/'
      var fn = function() {
        return makeSyncRequest(url, Value.encodeValue([].slice.call(arguments,
              0)));
      };
      fn.async = function() {
        var callback = arguments[arguments.length - 1];
        var args = [].slice.call(arguments, 0, arguments.length - 1);
        makeAsyncRequest(url, Value.encodeValue(args), callback);
      };
      return fn;
    });
  }

  function bufferToUInt8Array(buffer) {
    var bytesArray = new Uint8Array(buffer.length);
    for (var i = 0; i < buffer.length; i++) {
      bytesArray[i] = buffer.readUInt8(i);
    }
    return bytesArray;
  }

  function UInt8ArrayToBuffer(bytesArray) {
    console.log('bytesArray len', bytesArray.byteLength);
    var buffer = new Buffer(bytesArray.byteLength);
    for (var i = 0; i < bytesArray.byteLength; i++) {
      console.log('writing index ',i);
      console.log('writing value', bytesArray[i]);
      buffer.writeUInt8(bytesArray[i], i);
    }
    return buffer;
  }

  function makeSyncRequest(url, postdata) {
    throw new Error('sync requests not allowed!');
    var arr = bufferToUInt8Array(postdata || new Buffer(0));
    var req = new XMLHttpRequest();
    req.open('POST', url, false);
    req.responseType = 'arraybuffer';
    req.send(arr);
    if (req.status === 200) {
      return decodeData(UInt8ArrayToBuffer(req.response));
    } else {
      throw new Exception('Sync request to ' + url + ' failed - no 200 response'); 
    }
  }


  function makeAsyncRequest(url, postdata, callback) {
    var arr = bufferToUInt8Array(postdata || new Buffer(0));
    var req = new XMLHttpRequest();
    req.responseType = 'arraybuffer';
    req.open('POST', url, true);
    req.onload = function(e) {
      if (req.readyState === 4) {
        if (req.status === 200) {
          console.log('req.response', req.response);
          var responseBuffer = new Uint8Array(req.response);
          var binary = UInt8ArrayToBuffer(responseBuffer);
          callback(null, decodeData(binary));
        } else {
          callback(new Exception('Async request to ' + url + ' failed - no 200' +
              'response'));
        }
      } else {
        callback(new Exception('Async request to ' + url + ' failed - request ' +
            ' did not reach a ready state'));
      }
    }
    req.send(arr);
  }

  var path = window.location.pathname;
  path = path.substr(path.substr(1).indexOf('/') + 1);
  makeAsyncRequest('/' + getHashURL() + '/_api/apiObject', new Buffer(path, 'utf8'), function(err, api) {
    if (err) console.error(err);
    window.api = api;
  });

  window.Buffer = Buffer;
  window.UInt8ArrayToBuffer = UInt8ArrayToBuffer;
  window.getHashURL = getHashURL;
})();
